...
void data_entry_dialog(WINDOW *sub1, WINDOW *a, WINDOW *b)
{
    ...
    do
    {
        ...
        /* Проверка корректности введенного значения диаметра */
    ...
    enter_thick_info(a, 5, info_thick_first_part, ch, thick_first_1, thick_first_2);
...
double enter_force_y(WINDOW *sub1, WINDOW *a, int color_pair)
{
    ...
    waddch(sub1, ACS_DIAMOND);
    wmove(sub1, 3, 33);
    wprintw(sub1, "Y force is %.2f kN", force_y);
    wrefresh(sub1);
    return force_y;
}
